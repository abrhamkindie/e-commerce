      ...
      <div className="container mx-auto flex items-center justify-between px-6 py-4">
        ...
        <nav className="hidden md:flex space-x-8">
          {['Home', 'Shop', 'Categories', 'About', 'Contact'].map((item) => (
            <Link
              ...
              onClick={handleNavClick}
              className={`relative text-lg font-medium transition-colors ${
                activeItem === item ? 'text-yellow-300' : 'text-white hover:text-yellow-300'
              }`}
            >
              {item}
              {activeItem === item && (
        ...
        <div className="hidden md:flex items-center space-x-6">
          ...
          >
            <ShoppingCart
              ...
              className={`transition-colors duration-200 ${
                activeItem === 'Cart' ? 'text-yellow-300' : 'text-white hover:text-yellow-300'
              }`}
            />
            {cartItemCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-yellow-300 text-indigo-900 text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
          ...
          <Link
            to="/login"
            onClick={handleNavClick}
            className={`font-semibold px-4 py-1 rounded-md transition-colors duration-200 ${
              activeItem === 'Login' 
      ...
      {isOpen && (
        <div className="md:hidden bg-indigo-700 py-4 px-6 space-y-4 animate-slide-down">
          ...
          <Link
            to="/login"
            onClick={handleNavClick}
            className={`block font-semibold px-4 py-1 rounded-md transition-colors duration-200 ${
              activeItem === 'Login' 
                ? 'bg-yellow-300 text-indigo-900'
                : 'bg-yellow-300 text-indigo-900 hover:bg-yellow-400'
            }`}
